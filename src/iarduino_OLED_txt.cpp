...
void	iarduino_OLED_txt::_begin(void){																				//	Параметр: отсутствует
...
void	iarduino_OLED_txt::setFont(const uint8_t* font){																//	Параметр:  название шрифта.
			insFont.font		=	(uint8_t*) font;																	//	Сохраняем указатель на первый байт массива в области памяти программ.
...
//		Вывод числа:																									//	Возвращаемое значение: отсутствует.
...
void	iarduino_OLED_txt::print(uint32_t num, int col, int str, int sys){												//	Параметры: число, № колонки, № строки (страницы), система счисления.
			...
			print(k, col, str);																							//	Выводим строку k.
																														...
																														//
//		Вывод символа:																									//	Возвращаемое значение: отсутствует.
void	iarduino_OLED_txt::print(char sym, int col, int str){															//	Параметры: символ, № колонки, № строки (страницы).
			char i[2]={sym,0}; _print(i, col, str);																		//	Вызываем функцию вывода текста _print().
}																														//
																														//
...
void	iarduino_OLED_txt::_sendCommand(uint8_t command){																//	Параметр: код команды.
			selI2C->writeByte(dispAddr, SSD1306_COMMAND, command);														//	Отправляем 3 байта: адрес dispAddr с битом rw=0 (запись), SSD1306_COMMAND, command.
}																														//
																														//
...
void	iarduino_OLED_txt::_sendData(uint8_t* data, uint8_t sum){														//	Параметры: указатель на начало массива данных, количество передаваемых байт
			for(uint8_t i=0; i<sum; i+=30){																				//	Передаём не более 30 байт за раз (буфера Wire.h для некоторых плат ограничен 32 байтами).
				selI2C->writeBytes(dispAddr, SSD1306_DATA, &data[i], ((i+30)<sum)?30:(sum-i) );							//	Передаём данные многобайтным пакетом, но не более 30 байт в одном пакете.
			}																											//
}																														//